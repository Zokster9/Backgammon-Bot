...
def heuristic_value(game_board):
    ...
    return 20 * difference_num_checkers + 2 * difference_num_bar_checkers + 5 * difference_num_checkers_home_board + 5 * difference_num_of_points + difference_num_of_blots
...
def reverse(reversed_moves):
    keys = []
    items = []
    moves = {}
    for k, i in reversed_moves.items():
        keys.append(k)
        items.append(i)
    keys.reverse()
    for i in range(0, len(keys)):
        moves[keys[i]] = items[i]
    return moves
    ...
    if node_type == 'MAX':
        ...
        if first_time:
            ...
            for moves in valid_moves.moves:
                try:
                    reversed_moves = dict(reversed(list(moves.items())))
                    done_moves.index(reversed_moves)
                    continue
                except ValueError:
                    done_moves.append(moves)
        ...
        chance_moves = Moves()
        if maximizing_player:
            chance_moves.generate_moves(game_board, game_dice, Figures.black_figure())
        else:
            chance_moves.generate_moves(game_board, game_dice, Figures.white_figure())
        if len(chance_moves.moves) == 0:
            if maximizing_player:
                return -100
            else:
                return 100
            ...
            done_moves = []
            for moves in chance_moves.moves:
                try:
                    if game_dice.die1 == game_dice.die2:
                        reversed_moves = reverse(dict(reversed(list(moves.items()))))
                    else:
                        reversed_moves = dict(reversed(list(moves.items())))
                    done_moves.index(reversed_moves)
                    continue
